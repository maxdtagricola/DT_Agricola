...
function atualizarUiAutenticacao(usuario) {
    ...
    // Mecanismo genérico: qualquer elemento marcado com
    // data-requer-login só aparece para quem estiver autenticado
    // (ex: a ferramenta de importação em massa). O inverso,
    // data-requer-deslogado, é para avisos como "faça login para continuar".
    document.querySelectorAll("[data-requer-login]").forEach(el => {
        el.style.display = usuario ? "" : "none";
    });

    document.querySelectorAll("[data-requer-deslogado]").forEach(el => {
        el.style.display = usuario ? "none" : "";
    });
